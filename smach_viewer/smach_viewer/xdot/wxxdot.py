...
class WxDotWindow(wx.Panel):
    ...
    def OnScroll(self, event):
        ...
        if event.GetWheelRotation() > 0:
            self.zoom_image(self.zoom_ratio * self.ZOOM_INCREMENT, pos=(event.GetX(), event.GetY()))
        else:
            self.zoom_image(self.zoom_ratio / self.ZOOM_INCREMENT, pos=(event.GetX(), event.GetY()))
    ...
    def zoom_to_area(self, x1, y1, x2, y2):
        ...
        self.zoom_ratio = min(float(width) / float(area_width), float(height) / float(area_height))
        self.zoom_to_fit_on_resize = False
        self.x = (x1 + x2) / 2
        self.y = (y1 + y2) / 2
        self.Refresh()
    ...
    def set_dotcode(self, dotcode, filename="<stdin>"):
        ...
        if p.returncode != 0:
            ...
            dialog = Gtk.MessageDialog(
                type=Gtk.MESSAGE_ERROR, message_format=error, buttons=Gtk.BUTTONS_OK
            )
